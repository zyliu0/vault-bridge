...
`/vault-bridge:viz`, placed either inside a project folder or in the
...
`/vault-bridge:viz` generates a standalone visual artifact from a plain-text
...
3. `_Viz/` fallback — vault-level folder for standalone viz artifacts
...
chars on topic, hyphen-boundary truncation). Computed by `scripts/viz_naming.py`.

**Memory reporting:** viz runs write to `.vault-bridge/reports/` with scan
